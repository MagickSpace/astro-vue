import { defaultLang, showDefaultLang, ui } from './config.js'
...
export function generateAlternateLink(path, language, url) {
	...
	return url + translatePath(pathWithoutPrefix, language)
}

/**
 * Interpolates a localized string (loaded with the i18nKey) to a given reference string.
 */
export const interpolate = (i18n, referenceString, lang) => {
	...
	const tempReferenceString = referenceString.replace(/<br\s*(?:[^>]*)\/?>/gi, placeholder)
	const tempLocalizedString = localizedString.replace(/<br\s*\/?>/gi, placeholder)
	...
	interpolatedString = interpolatedString.replace(new RegExp(placeholder, 'g'), '<br />')
